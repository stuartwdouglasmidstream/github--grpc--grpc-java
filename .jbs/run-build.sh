...
export PROJECT_VERSION=1.38.1
export JAVA_HOME=/lib/jvm/java-1.8.0
export ENFORCE_VERSION=

set -- "$@" assemble publishToMavenLocal -DdisableTests -PskipAndroid=true -PskipCodegen=true 
...
if [ -z "" ]; then
  echo "Enforce version not set, recreating original tag v1.38.1"
  git tag -m v1.38.1 -a v1.38.1 || true
else
